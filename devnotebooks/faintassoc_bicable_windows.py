...
n_ds = xr.load_dataset(
    "../out/sparse_picks_Gabor/peaks_indexes_tp_North_2021-11-04_02:00:02_ipi3_th_4.nc"
)
s_ds = xr.load_dataset(
    "../out/sparse_picks_Gabor/peaks_indexes_tp_South_2021-11-04_02:00:02_ipi3_th_5.nc"
)
...
ax.add_patch(
    Rectangle(
        (np.min(n_dist / 1e3), np.min(s_cable_pos[:, 2])),
        56000 / 1e3 - np.min(n_dist / 1e3),
        -np.min(s_cable_pos[:, 2]),
        edgecolor="black",
        facecolor="tab:grey",
        fill=True,
        alpha=0.3,
...
ax.add_patch(
    Rectangle(
        (35000 / 1e3, np.min(s_cable_pos[:, 2])),
        np.max(n_dist / 1e3) - 35000 / 1e3,
        -np.min(s_cable_pos[:, 2]),
        edgecolor="tab:blue",
        facecolor="tab:blue",
        alpha=0.3,
...
ax.add_patch(
    Rectangle(
        (56000 / 1e3, np.min(s_cable_pos[:, 2])),
        np.max(s_dist / 1e3) - 56000 / 1e3,
        -np.min(s_cable_pos[:, 2]),
        edgecolor="tab:green",
        facecolor="tab:green",
        alpha=0.3,
...
def plot_cables2D_m_rectPatches(df_north, df_south, bathy, xm, ym):
    ...
    ax.add_patch(
        Rectangle(
            (np.min(df_north["x"]), np.min(df_south["y"])),
