...
def main(urls, selected_channels_m):
    ...
    # South cable plots
        ...
        metadata = dw.data_handle.get_acquisition_parameters(
            filepaths[0], interrogator="optasense"
        )
        ...
        selected_channels = dw.data_handle.get_selected_channels(
            selected_channels_m, dx
        )
        ...
        # Load the data
        tr, time, dist, fileBeginTimeUTC = dw.data_handle.load_mtpl_das_data(
            filepaths, selected_channels, metadata, timestamp, duration
        )
        metadata["fileBeginTimeUTC"] = fileBeginTimeUTC.strftime("%Y-%m-%d_%H:%M:%S")
        metadata["data_shape"] = tr.shape
    ...
    fk_filter_noise = dw.dsp.hybrid_ninf_gs_filter_design(
        (tr.shape[0], tr.shape[1]),
        selected_channels,
        dx,
        fs,
    ...
    SNR_hf = 20 * np.log10(
        abs(sp.hilbert(nmf_m_HF, axis=1)) / abs(sp.hilbert(noise, axis=1))
    )
    SNR_lf = 20 * np.log10(
        abs(sp.hilbert(nmf_m_LF, axis=1)) / abs(sp.hilbert(noise, axis=1))
    )
    ...
    ds.to_netcdf(
        f"out/peaks_indexes_tp_{metadata['cablename']}_{metadata['fileBeginTimeUTC']}_ipi{ipi}_th_{th}.nc"
    )
    ...
    plt.scatter(
        peaks_indexes_tp_HF[1] / fs,
        (peaks_indexes_tp_HF[0] * selected_channels[2] + selected_channels[0])
        * dx
        / 1e3,
        c=flat_snr_hf,
        cmap=cmap,
    ...
    plt.savefig(
        f"figs/tpicks/Peaks_HF_{metadata['cablename']}_{metadata['fileBeginTimeUTC']}_ipi{ipi}_th_{th}.pdf"
    )
    # plt.show()

    plt.figure(figsize=(12, 10))
    plt.scatter(
        peaks_indexes_tp_LF[1] / fs,
        (peaks_indexes_tp_LF[0] * selected_channels[2] + selected_channels[0])
        * dx
        / 1e3,
        c=flat_snr_lf,
        cmap=cmap,
    ...
    plt.savefig(
        f"figs/tpicks/Peaks_LF_{metadata['cablename']}_{metadata['fileBeginTimeUTC']}_ipi{ipi}_th_{th}.pdf"
    )
    # plt.tight_layout()
    # plt.show()
