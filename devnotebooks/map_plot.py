...
def map_plot():
    ...
    utm_x0, utm_y0 = dw.map.latlon_to_utm(
        xlon[0], ylat[0]
    )  # UTM of the first grid point
    utm_xf, utm_yf = dw.map.latlon_to_utm(
        xlon[-1], ylat[-1]
    )  # UTM of the last grid point
    ...
    ax.plot_surface(
        np.tile(X_plane, (2, 1)),
        np.tile(Y_plane, (2, 1)),
        np.vstack([np.full_like(Z_plane, depth_min), Z_plane]),
        color="sandybrown",
        alpha=1,
        edgecolor="none",
        rasterized=True,
    ...
    ax.contour(
        ...
        Y,
        bathy,
        levels=[-1400, -1200, -1000, -800, -600, -400, -300, -200, -100, 0],
        colors="k",
        linestyles="solid",
        linewidths=0.5,
    )
    ...
    # Pacific City
    ax.scatter(
        utm_x_pc / 1e3, utm_y_pc / 1e3, 10, color="black", s=1000, marker="*", zorder=10
    )
    ax.text(
        utm_x_pc / 1e3,
        utm_y_pc / 1e3,
    ...
    # 1st repeaters
    ax.scatter(
        df_north["x"].iloc[-1] / 1e3,
        df_north["y"].iloc[-1] / 1e3,
        df_north["depth"].iloc[-1],
        color="tab:red",
        s=300,
        label="1st repeater",
        marker="s",
    ...
    ax.scatter(
        df_south["x"].iloc[-1] / 1e3,
        df_south["y"].iloc[-1] / 1e3,
        df_south["depth"].iloc[-1],
        color="tab:orange",
        s=300,
        label="1st repeater",
        marker="s",
