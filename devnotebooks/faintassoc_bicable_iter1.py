...
axes[0, 0].set_ylim(
    min(n_selected_channels_m[0] + npeakshf[0][:] * dx) * 1e-3,
...
axes[0, 1].set_ylim(
    min(n_selected_channels_m[0] + npeakslf[0][:] * dx) * 1e-3,
...
axes[1, 0].set_ylim(
    min(s_selected_channels_m[0] + speakshf[0][:] * dx) * 1e-3,
...
axes[1, 1].set_ylim(
    min(s_selected_channels_m[0] + speakslf[0][:] * dx) * 1e-3,
...
plt.savefig(
    "../figs/Sparse_peaks.pdf", bbox_inches="tight", transparent=True, format="pdf"
)
plt.show()
...
fig, (ax1, ax2) = plt.subplots(
    2, 1, figsize=(8, 20), sharex=True, constrained_layout=True
)
...
hist, bin_edges = np.histogram(
    n_delayed_picks_hf[nhf_imax, :], bins=bins, range=hist_range, weights=nSNRhf
)
hist = sp.convolve(hist, kernel, mode="same")
hist = hist / np.trapezoid(hist, t_kde)

s_hist, s_bin_edges = np.histogram(
    s_delayed_picks_hf[shf_imax, :], bins=bins, range=hist_range, weights=sSNRhf
)
s_hist = sp.convolve(s_hist, kernel, mode="same")
s_hist = s_hist / np.trapezoid(s_hist, t_kde)

lf_hist, lf_bin_edges = np.histogram(
    n_delayed_picks_lf[nlf_imax, :], bins=bins, range=hist_range, weights=nSNRlf
)
lf_hist = sp.convolve(lf_hist, kernel, mode="same")
lf_hist = lf_hist / np.trapezoid(lf_hist, t_kde)
s_lf_hist, s_lf_bin_edges = np.histogram(
    s_delayed_picks_lf[slf_imax, :], bins=bins, range=hist_range, weights=sSNRlf
)
...
plt.grid(linestyle="--", alpha=0.5)
plt.savefig(
    "../figs/delayed_picks_HF.pdf", bbox_inches="tight", transparent=True, format="pdf"
)
plt.show()
# -
...
plt.grid(linestyle="--", alpha=0.5)
plt.savefig(
    "../figs/delayed_picks_LF.pdf", bbox_inches="tight", transparent=True, format="pdf"
)
plt.show()
...
axes[0, 0].plot(
    max_time_hf + n_arr_tg[hf_imax, :], n_dist / 1e3, ls="-", lw=3, color="tab:blue"
)
axes[0, 0].plot(
...
axes[0, 1].plot(
    max_time_lf + n_arr_tg[lf_imax, :], n_dist / 1e3, ls="-", lw=3, color="tab:blue"
)
axes[0, 1].plot(
...
axes[1, 0].plot(
    max_time_hf + s_arr_tg[hf_imax, :], s_dist / 1e3, ls="-", lw=3, color="tab:blue"
)
axes[1, 0].plot(
...
axes[1, 1].plot(
    max_time_lf + s_arr_tg[lf_imax, :], s_dist / 1e3, ls="-", lw=3, color="tab:blue"
)
axes[1, 1].plot(
...
# Create figure
fig, axes = plt.subplots(
    2, 1, figsize=(10, 16), sharex=True, sharey=False, constrained_layout=True
)
...
def associate_picks(
    ...
    hyperbola = (
        max_time + arr_tg[imax, :]
    )  # Theoretical arrival times for the selected hyperbola
    idx_dist, idx_time = compute_selected_picks(
        up_peaks, hyperbola, dt_sel, fs
    )  # Select the picks around the hyperbola within +/- dt_sel
    ...
    n, residuals = dw.loc.solve_lq(
        Ti, cable_pos[idx_dist], c0, Nbiter, fix_z=True, ninit=n_init, residuals=True
    )
    # rms residual
    ...
    if rms < 0.4:
        ...
        mask_resi = (
            mod_cs < 1500
        )  # Mask the residuals that are below the threshold, key parameter
        ...
        for d, t in zip(
            idx_dist[mask_resi], idx_time[mask_resi]
        ):  # For each pair to remove, update the mask
...
for iteration in pbar:
    ...
    pbar.set_description(
        f"Associated calls: {len(n_associated_list) + len(s_associated_list)}"
    )
...
def plot_reject_pick(
    peaks, longi_offset, dist, dx, associated_list, rejected_list, rejected_hyperbolas
):
...
# Example usage:
fig = plot_pick_analysis(
    n_associated_list[:10], fs, dx, n_longi_offset, n_cable_pos, n_dist
)
fig = plot_pick_analysis(
    s_associated_list[:10], fs, dx, s_longi_offset, s_cable_pos, s_dist
)
plt.show()
...
n_localizations, n_alt_localizations = loc_from_picks(
    n_associated_list, n_cable_pos, c0, fs
)
s_localizations, s_alt_localizations = loc_from_picks(
    s_associated_list, s_cable_pos, c0, fs
)


# +
def plot_associated(
    peaks, longi_offset, associated_list, localizations, cable_pos, dist, dx, c0
):
    plt.figure(figsize=(20, 8))
...
for i in range(
    int(idx_step - df_south["chan_idx"].iloc[0]), len(df_south), int(10000 / 2)
):
...
contour_dashed = ax.contour(
    bathy, levels=depth_levels, colors="k", linestyles="--", extent=extent, alpha=0.6
)
...
for i, loc in enumerate(n_localizations):
    ...
    if i == 0:
        ax.plot(
            loc[0], loc[1], "o", c="tab:purple", lw=4, label="Localized call - north"
        )
    else:
...
for i, loc in enumerate(s_localizations):
    ...
    if i == 0:
        ax.plot(
            loc[0], loc[1], "o", c="tab:green", label="Localized call - south", lw=4
        )
    else:
