...
def main(url):
    ...
    # detect_calls.xcorr()
    # ```
    spectro, ff, tt = dw.detect.get_sliced_nspectrogram(
        trf_fk[xi_m], fs, fm_corr, fM_corr, nperseg, nhop, plotflag=True
    )
    tvec, fvec_sub, Finkernel_hf = dw.detect.buildkernel(
        f0_hf,
        f1_hf,
        bandwidth,
        duration_hf,
    ...
    tvec, _, Finkernel_lf = dw.detect.buildkernel(
        f0_lf,
        f1_lf,
        bandwidth,
        duration_lf,
    ...
    plt.imshow(
        spectro[:, testidx : testidx + int(6 * fs_spectro)],
        aspect="auto",
        origin="lower",
        cmap=roseus,
        extent=[tt[testidx], tt[testidx + int(6 * fs_spectro)], ff[0], ff[-1]],
        vmin=-0.5,
        vmax=1,
    )
    int(10 * fs)
    ...
    flims = [fmin, fmax]
    kernelHF = {"f0": 27.0, "f1": 17.0, "dur": 0.8, "bdwidth": 4.0}
    kernelLF = {"f0": 20.0, "f1": 14.0, "dur": 1.2, "bdwidth": 4.0}
    correlogram_HF = dw.detect.compute_cross_correlogram_spectrocorr(
        trf_fk, fs, flims, kernelHF, window_size, overlap_pct
    )
    correlogram_LF = dw.detect.compute_cross_correlogram_spectrocorr(
        trf_fk, fs, flims, kernelLF, window_size, overlap_pct
    )
    ...
    dw.plot.plot_cross_correlogramHL(
        correlogram_HF, correlogram_LF, time, dist, 0.5 * maxv
    )
