...
def locs_to_rows(
    ...
    rows = []
    for (x_loc, y_loc, z_loc, t), (x_utm, y_utm, _, _), (lon, lat, _, _) in zip(
...
def process_one_file(
    pkl_path: Path, north_csv: str, south_csv: str, utm_xf: float, utm_y0: float
) -> list:
    ...
    for key, data in loc_dict.items():
        ...
        rows.extend(
            locs_to_rows(data["local"], data["utm"], data["latlon"], sensor, call, utc0)
        )
    return rows


def process_all(
    pkl_dir: str, north_csv: str, south_csv: str, utm_xf: float, utm_y0: float
) -> pd.DataFrame:
    all_rows = []
    for pkl in sorted(Path(pkl_dir).glob("association_*.pkl")):
...
dist_matrix = spa.distance_matrix(
    df_all[["x_local", "y_local"]].to_numpy(), df_all[["x_local", "y_local"]].to_numpy()
)
...
norm = mcolors.Normalize(vmin=df_valid["minutes"].min(), vmax=df_valid["minutes"].max())
...
plt.savefig(
    "localization_kilometers.pdf", format="pdf", bbox_inches="tight", transparent=True
)
...
for (sensor, call), grp in groups:
    ...
    m = markers.get(lbl)
    sc = ax.scatter(
        [], [], color=mpl.colormaps["plasma"](0), marker=m, s=100, label=lbl
    )
    artists[lbl] = sc
ax.legend(loc="upper right", fontsize="small")
...
for (sensor, call), grp in groups:
    lbl = f"{sensor}-{call}"
    m = markers.get(lbl)
    sc = ax.scatter([], [], c=[], cmap="plasma", norm=norm, marker=m, s=90, label=lbl)
    artists[lbl] = sc
...
def update(frame_time):
    ...
    for (sensor, call), _ in groups:
        ...
        sc = artists[lbl]
        df_grp = df_frame[
            (df_frame["sensor"] == sensor) & (df_frame["call_type"] == call)
        ]
...
anim = FuncAnimation(
    fig, update, frames=times[::2], init_func=init, blit=True, interval=1000 / 35
)

writer = FFMpegWriter(fps=35, bitrate=2400)
anim.save(
    "localization_animation.mp4",
    writer=writer,
    dpi=300,
