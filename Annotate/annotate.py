...
north_files = sorted(
    glob.glob(os.path.join("..", output_dir, "Denoised_SNR_North_*.nc"))
)
south_files = sorted(
    glob.glob(os.path.join("..", output_dir, "Denoised_SNR_South_*.nc"))
)
...
def make_existing_dmap(cable, freq):
    ...
    def _read_and_filter():
        ...
        return hv.Points(df, kdims=["time", "dist"], vdims=list(df.columns)).opts(
            size=6, color="yellow", marker="o"
        )

    return hv.DynamicMap(lambda **kwargs: _read_and_filter(), streams=[reload_stream])
...
def create_annotator(snr_data, title, cable, freq):
    ...
    base = snr_data.hvplot(
        ...
        ylabel="Distance (km)",
        colormap="turbo",
        colorbar=False,  # Using a non-perceptually uniform colormap is actually better here because it highlights faint parts of the signal and noise. Inferno and viridis are good to see mostly the High SNR parts.
    ...
    def on_tap(x, y):
        ...
        return hv.Points(filt, kdims=["time", "dist"], vdims=list(filt.columns)).opts(
            size=6, color="magenta", tools=["hover"], marker="o"
        )
...
message = pn.pane.Markdown(
    "", width=300, styles={"font-size": "18px", "font-weight": "bold"}
)
...
def save_current_call(event):
    ...
    if not north.empty:
        _append(north, output_north)
    if not south.empty:
        _append(south, output_south)
