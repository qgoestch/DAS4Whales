...
def main(url):
    ...
    dw.plot.plot_3calls(trf_fk[xi_m], time, 6.0, 27.6, 48.5)
    # Generate fin whale call template
    tpl_paper = dw.detect.gen_template_fincall(
        time, fs, fmin=15.0, fmax=30.0, duration=2.25, window=False
    )

    HF_note = dw.detect.gen_template_fincall(
        time, fs, fmin=17.8, fmax=28.8, duration=0.68
    )
    LF_note = dw.detect.gen_template_fincall(
        time, fs, fmin=14.7, fmax=21.8, duration=0.78
    )
    template = HF_note

    dw.plot.design_mf(trf_fk[xi_m], HF_note, LF_note, 6.17, 28.0, time, fs)
    ...
    thres = 0.5 * maxv
    print(
        f" The detection threshold is {thres} for the low frequency note and {thres * 0.9} for the high frequency note."
    )
