...
for iteration in pbar:
    ...
    nhf_idx_dist, nhf_idx_time = dw.assoc.select_picks(
        n_up_peaks_hf, nhf_hyperbola, dt_sel, fs
    )
    shf_idx_dist, shf_idx_time = dw.assoc.select_picks(
        s_up_peaks_hf, shf_hyperbola, dt_sel, fs
    )
    nlf_idx_dist, nlf_idx_time = dw.assoc.select_picks(
        n_up_peaks_lf, nlf_hyperbola, dt_sel, fs
    )
    slf_idx_dist, slf_idx_time = dw.assoc.select_picks(
        s_up_peaks_lf, slf_hyperbola, dt_sel, fs
    )
    ...
    nhf_n, nhf_residuals = dw.assoc.loc_picks(
        nhf_idx_dist, nhf_idx_time, n_cable_pos, c0, fs
    )
    shf_n, shf_residuals = dw.assoc.loc_picks(
        shf_idx_dist, shf_idx_time, s_cable_pos, c0, fs
    )
    nlf_n, nlf_residuals = dw.assoc.loc_picks(
        nlf_idx_dist, nlf_idx_time, n_cable_pos, c0, fs
    )
    slf_n, slf_residuals = dw.assoc.loc_picks(
        slf_idx_dist, slf_idx_time, s_cable_pos, c0, fs
    )
    ...
    if hf_north_south_good and lf_north_south_good:
        ...
        if max_time_hf >= 0:  # Do not associate the edge cases
            ...
            mask_resi_n_hf = dw.assoc.filter_peaks(
            ...
            mask_resi_s_hf = dw.assoc.filter_peaks(
        ...
        if max_time_lf >= 0:  # Do not associate the edge cases
            ...
            mask_resi_n_lf = dw.assoc.filter_peaks(
            ...
            mask_resi_s_lf = dw.assoc.filter_peaks(
        ...
        n_up_peaks_hf, nSNRhf = dw.assoc.remove_peaks_tolerance(
            n_up_peaks_hf,
            nlf_idx_dist,
            nlf_idx_time,
            mask_resi_n_lf,
        ...
        n_up_peaks_lf, nSNRlf = dw.assoc.remove_peaks_tolerance(
            n_up_peaks_lf,
            nhf_idx_dist,
            nhf_idx_time,
            mask_resi_n_hf,
        ...
        s_up_peaks_hf, sSNRhf = dw.assoc.remove_peaks_tolerance(
            s_up_peaks_hf,
            slf_idx_dist,
            slf_idx_time,
            mask_resi_s_lf,
        ...
        s_up_peaks_lf, sSNRlf = dw.assoc.remove_peaks_tolerance(
            s_up_peaks_lf,
            shf_idx_dist,
            shf_idx_time,
            mask_resi_s_hf,
    ...
    # First priority: Case 1 - HF North and South are good
        ...
        if max_time_hf >= 0:  # Do not associate the edge cases
            mask_resi_n = dw.assoc.filter_peaks(
                nhf_residuals, nhf_idx_dist, nhf_idx_time, n_longi_offset, dx
            )
            ...
            mask_resi_s = dw.assoc.filter_peaks(
                shf_residuals, shf_idx_dist, shf_idx_time, s_longi_offset, dx
            )
    ...
    if not processed:
        ...
        if only_hf_north_good:
            if max_time_hf >= 0:
                mask_resi = dw.assoc.filter_peaks(
                    nhf_residuals, nhf_idx_dist, nhf_idx_time, n_longi_offset, dx
                )
...
def plot_pick_analysis(
    ...
    for i, select in enumerate(associated_list):
        ...
        ax.scatter(times, distances, label="All Picks", c="gray", alpha=0.5, s=1)

        idxmin_t = np.argmin(select[1][:])
        mask_dist = (
            abs(distances - distances[idxmin_t]) < 40
        )  # limit the distance to 40 km from the minimum
        ...
        ax.scatter(
            times[mask_resi], distances[mask_resi], label="HF", s=4, color="tab:blue"
        )
        ax.scatter(
            select[1][mask_dist] / fs,
            (longi_offset + select[0][mask_dist]) * dx * 1e-3,
            label="LF",
            s=8,
            color="tab:orange",
        )
...
fig = plot_pick_analysis(
    nhf_assoc_list, fs, dx, n_longi_offset, n_cable_pos, n_dist, nSNRhf, npeakshf
)
plt.show()
...
fig = plot_pick_analysis(
    s_rejected_list[:5], fs, dx, s_longi_offset, s_cable_pos, s_dist, sSNRhf, speakshf
)
fig = plot_pick_analysis(
    n_rejected_list[:5], fs, dx, n_longi_offset, n_cable_pos, n_dist, nSNRhf, npeakshf
)
plt.show()
...
def loc_picks_bicable(n_assoc, s_assoc, cable_pos, c0, fs, Nbiter=20):
    ...
    n, residuals = dw.loc.solve_lq_weight(
        times, bicable_pos, c0, Nbiter, fix_z=True, ninit=init, residuals=True
    )

    return n, residuals


n_test, res = loc_picks_bicable(
    nhf_assoc_list_pair[0], shf_assoc_list_pair[0], bicable_pos, c0, fs
)
...
hf_pair_loc, hf_alt_loc = loc_picks_bicable_list(
    nhf_assoc_list_pair, shf_assoc_list_pair, bicable_pos, c0, fs
)
lf_pair_loc, lf_alt_loc = loc_picks_bicable_list(
    nlf_assoc_list_pair, slf_assoc_list_pair, bicable_pos, c0, fs
)
...
def loc_from_picks_list(associated_list, cable_pos, c0, fs, peaks, snr):
    ...
    for select in associated_list:
        ...
        # Solve the least squares problem
        n = dw.loc.solve_lq(
            Ti, cable_pos[select[0][:]], c0, Nbiter, SNR=snr_i, fix_z=True, ninit=n_init
        )
