...
def energy_TimeDomain(da: xr.DataArray, time_dim: str = "time") -> xr.DataArray:
    ...
    template = xr.DataArray(
        dask.array.random.random(output_sizes, chunks=list(new_chunk_sizes.values())),
        dims=da.dims,
        name=f"energy in {time_dim} dimension",
    )

    da_energy = da.map_blocks(
        _energy_TimeDomain_chunk, template=template, kwargs={"time_dim": time_dim}
    )
...
def filtfilt_chunk(da: xr.DataArray, dim: str = "time", **kwargs) -> xr.DataArray:
    ...
    **kwargs : various types
    ...
    da_filtx = xr.DataArray(
        da_filt, dims=da.dims, coords=da.coords, name=da.name, attrs=da.attrs
    )
...
def spec(da: xr.DataArray) -> xr.DataArray:
    ...
    template = xr.DataArray(
        np.ones((int(da.sizes["time"] / 3000), 513)), dims=["time", "frequency"]
    ).chunk({"time": 1, "frequency": 513})
...
def disp_comprate(fk_filter: Any) -> None:
    ...
    print(
        f"The compression ratio is {sizefilt / size_sprfilt_coo:.2f} ({abs(sizefilt - size_sprfilt_coo) * 100 / sizefilt:.1f} %)"
    )
