...
def generate_hourly_plots(
    ...
    spatial_proximity_m: float = 250.0,
    ...
    print(
        f"Generating plots for {n_windows} windows of {window_minutes} minutes each..."
    )
    ...
    for w in range(n_windows):
        ...
        if spatial_proximity_m is not None and len(df_win) > 1:
            try:
                dist_matrix = spa.distance_matrix(
                    df_win[["x_local", "y_local"]].to_numpy(),
                    df_win[["x_local", "y_local"]].to_numpy(),
                )
        ...
        if len(df_win) > 0 and (min_time_spacing_s is not None):
            ...
            for i in range(1, len(df_win)):
                time_diff = (
                    df_win.iloc[i]["utc"] - df_win.iloc[keep_indices[-1]]["utc"]
                ).total_seconds()
        ...
        median_deltax = (
            np.median(df_win["deltax"]) if "deltax" in df_win.columns else np.nan
        )
        ...
        markers = {"hf": "o", "lf": "d"}
        legend_name_map = {"hf": "HF calls", "lf": "LF calls"}
        ...
        ax.set_title(
            f"UTC window: {t0.strftime('%Y-%m-%d %H:%M')} to {t1.strftime('%Y-%m-%d %H:%M')}"
        )
        ...
        out_path = Path(out_dir) / f"tracks_{t0.strftime('%Y%m%d_%H%M')}.pdf"
        fig.savefig(
            str(out_path), format="pdf", bbox_inches="tight", transparent=True, dpi=200
        )
        plt.close(fig)
...
def filter_global(
    ...
    out_dir: str,
    window_minutes: int,
    deltax_threshold: float,
    spatial_proximity_m: float,
    time_proximity_s: int,
):
