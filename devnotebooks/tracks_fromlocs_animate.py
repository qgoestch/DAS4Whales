...
if num_inverted > 0:
    logger.info(
        f"Found {num_inverted} pair-located calls with inverted sensor/call_type. Swapping..."
    )
    df_all.loc[mask, ["call_type", "sensor"]] = df_all.loc[
        mask, ["sensor", "call_type"]
    ].values
...
def generate_track_animation(
    ...
    deltax_threshold: float = None,
    spatial_proximity_m: float = None,
    time_proximity_s: int = None,
    ...
    (south_cplot,) = ax.plot(
        df_south.x_km, df_south.y_km, c="orange", label="South cable"
    )

    # Add legend and empty markers for HF and LF
    hf_marker = plt.scatter(
        [], [], c="gold", marker="o", edgecolor="k", s=50, label="HF calls"
    )
    lf_marker = plt.scatter(
        [], [], c="magenta", marker="d", edgecolor="k", s=50, label="LF calls"
    )
    ax.legend(
        handles=[north_cplot, south_cplot, hf_marker, lf_marker],
        loc="upper right",
        fontsize="small",
    )
    ...
    def update(frame):
        ...
        if spatial_proximity_m is not None and len(df_window) > 1:
            try:
                dist_matrix = spa.distance_matrix(
                    df_window[["x_local", "y_local"]].to_numpy(),
                    df_window[["x_local", "y_local"]].to_numpy(),
                )
                has_spatial_neighbor_mask = np.any(
                    (dist_matrix <= spatial_proximity_m) & (dist_matrix > 0), axis=1
                )
        ...
        if (
            len(df_window) > 0
            and (time_proximity_s is not None)
            and (spatial_proximity_m is not None)
        ):
            ...
            for ii, (_, row) in enumerate(df_window.iterrows()):
                ...
                left = int(
                    np.searchsorted(
                        all_times, t - np.timedelta64(int(time_proximity_s), "s")
                    )
                )
                right = int(
                    np.searchsorted(
                        all_times,
                        t + np.timedelta64(int(time_proximity_s), "s"),
                        side="right",
                    )
                )
                if right <= left:
        ...
        if len(df_window) > 0 and (min_time_spacing_s is not None):
            ...
            for i in range(1, len(df_window)):
                time_diff = (
                    df_window.iloc[i]["utc"] - df_window.iloc[keep_indices[-1]]["utc"]
                ).total_seconds()
        ...
        if df_window.empty:
            ax.set_title(
                f"Tracks {window_start.strftime('%Y-%m-%d %H:%M')} to {window_end.strftime('%Y-%m-%d %H:%M UTC')}"
            )
            return []
        ...
        df_hf = df_window[
            df_window["call_type"].str.contains("hf", case=False, na=False)
        ]
        df_lf = df_window[
            df_window["call_type"].str.contains("lf", case=False, na=False)
        ]
    ...
    anim = FuncAnimation(
        fig, update, frames=len(windows), init_func=init, blit=False, interval=interval
    )

    out_path = (
        Path(out_dir) / f"4d_{window_minutes}win_{overlap_minutes}overlap_tracks.mp4"
    )
...
generate_track_animation(
    ...
    overlap_minutes=88,
    deltax_threshold=80,
    spatial_proximity_m=250,
    time_proximity_s=70,
    min_time_spacing_s=5,
)
