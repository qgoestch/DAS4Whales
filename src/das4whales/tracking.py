...
def process_association_enhanced(
    pkl_path: Path,
    north_csv: str,
    south_csv: str,
    utm_xf: float,
    ...
    if p_n_hf and p_s_hf:
        results["hf_pair"] = loc_picks_bicable_list(
            p_n_hf, p_s_hf, (north_pos, south_pos), c0, fs, return_uncertainty=True
        )
    else:
        results["hf_pair"] = []

    if p_n_lf and p_s_lf:
        results["lf_pair"] = loc_picks_bicable_list(
            p_n_lf, p_s_lf, (north_pos, south_pos), c0, fs, return_uncertainty=True
        )
    else:
        results["lf_pair"] = []

    if n_hf:
        results["north_hf"] = loc_from_picks(
    ...
    if n_lf:
        results["north_lf"] = loc_from_picks(
            n_lf, north_pos, c0, fs, return_uncertainty=True
        )
    else:
        results["north_lf"] = []

    if s_hf:
        results["south_hf"] = loc_from_picks(
            s_hf, south_pos, c0, fs, return_uncertainty=True
        )
    else:
        results["south_hf"] = []

    if s_lf:
        results["south_lf"] = loc_from_picks(
            s_lf, south_pos, c0, fs, return_uncertainty=True
        )
    else:
        results["south_lf"] = []
    ...
    for key, loc_results in results.items():
        if loc_results:  # Only process if we have results
            processed_results[key] = convert_coordinates_enhanced(
                loc_results, utm_xf, utm_y0
            )
        else:
    ...
    return {
        ...
        "metadata": {"fs": fs, "c0": c0, "utm_ref": (utm_xf, utm_y0)},
    }
...
def convert_coordinates_enhanced(
    localization_results: list[LocalizationResult], utm_xf: float, utm_y0: float
) -> dict[str, list]:
        ...
        for sensor_call, data in file_data["results"].items():
            if data["results"]:  # Only process if we have results
                ...
                rows = localization_results_to_dict(
                    data["results"], utc_start=utc_start, sensor=sensor, call_type=call
                )
...
def plot_uncertainty_ellipses(df: pd.DataFrame, ax=None, scale_factor=1000, **kwargs):
    ...
    **kwargs
    ...
    for _, row in df.iterrows():
        if not (
            np.isnan(row["ellipse_semi_major"]) or np.isnan(row["ellipse_semi_minor"])
        ):
            ellipse = Ellipse(
