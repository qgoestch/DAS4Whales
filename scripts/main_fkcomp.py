...
def main(url):
    ...
    fk_filter_nf_cos = dw.dsp.hybrid_ninf_filter_design(
    ...
    fk_filter_nf_gs = dw.dsp.hybrid_ninf_gs_filter_design(
        (tr.shape[0], tr.shape[1]),
        selected_channels,
        dx,
        fs,
    ...
    fk_filter_inf_cos = dw.dsp.hybrid_filter_design(
        (tr.shape[0], tr.shape[1]),
        selected_channels,
        dx,
        fs,
    ...
    fk_filter_inf_gs = dw.dsp.hybrid_gs_filter_design(
        (tr.shape[0], tr.shape[1]),
        selected_channels,
        dx,
        fs,
    ...
    xi_m, tj_m = np.unravel_index(
        np.argmax(trf_fk_inf_cos, axis=None), trf_fk_inf_cos.shape
    )
    plt.figure()
    ...
    SNR_m = dw.dsp.snr_tr_array(
        trf_fk_inf_cos
    )  # 10 * np.log10(trf_fk_inf_cos ** 2 / np.std(trf_fk_inf_cos, axis=1, keepdims=True) ** 2)
