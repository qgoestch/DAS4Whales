...
def grid_2d_cross_correlation(
    n_times, n_dist, n_snr, s_times, s_dist, s_snr, time_bins=200, dist_bins=200
):
    """
    ...
    n_grid, _, _, _ = binned_statistic_2d(
        n_times, n_dist, n_snr, bins=[time_edges, dist_edges], statistic="mean"
    )
    s_grid, _, _, _ = binned_statistic_2d(
        s_times, s_dist, s_snr, bins=[time_edges, dist_edges], statistic="mean"
    )
...
im1 = axes[0, 0].imshow(
    n_grid.T,
    extent=[time_edges[0], time_edges[-1], dist_edges[0] * 1e-3, dist_edges[-1] * 1e-3],
    aspect="auto",
    origin="lower",
    cmap="viridis",
)
