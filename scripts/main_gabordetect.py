...
def main(url):
    ...
    fimage = cv2.filter2D(imagebin, cv2.CV_64F, gabfilt_up) + cv2.filter2D(
        imagebin, cv2.CV_64F, gabfilt_down
    )

    plt.figure(figsize=(10, 6))
    plt.imshow(
        fimage, aspect="equal", origin="lower", cmap="turbo", vmin=0
    )  # , vmin=-0.4 * np.max(np.abs(fimage)), vmax= 0.4 * np.max(np.abs(fimage)))
    ...
    mask = cv2.filter2D(
        binary_image.astype(float), cv2.CV_64F, gabfilt_up
    ) + cv2.filter2D(binary_image.astype(float), cv2.CV_64F, gabfilt_down)
    threshold2 = 150
    ...
    dw.plot.plot_3calls(masked_tr[xi_m], time, 6.0, 27.6, 48.5)
    # Spectrogram
    p, tt, ff = dw.dsp.get_spectrogram(
        masked_tr[xi_m + 3000, :], fs, nfft=256, overlap_pct=0.95
    )
    ...
    for i in tqdm(range(masked_tr.shape[0])):
        if np.max(masked_tr[i, :]) > 0:
            cross_correlogram_HF[i, :] = sp.correlate(
                masked_tr[i, :] / np.max(masked_tr[i, :]),
                HF_note,
                mode="same",
                method="fft",
            )
            cross_correlogram_LF[i, :] = sp.correlate(
                masked_tr[i, :] / np.max(masked_tr[i, :]),
                LF_note,
                mode="same",
                method="fft",
    ...
    dw.plot.plot_cross_correlogramHL(
        cross_correlogram_HF, cross_correlogram_LF, time, dist, maxv
    )
    ...
    peaks_indexes_m_HF = dw.detect.pick_times_env(
        cross_correlogram_HF, threshold=thres * 0.9
    )
