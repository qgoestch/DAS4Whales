...
for array, annots, sel_ch, sel_ch_m in [
    ...
    for call_type in ["hf", "lf"]:
        ...
        all_pairs = [
            get_time_dist(pairs, fs, dx, sel_ch, sel_ch_m)
            for pairs in pairs_list
            if pairs[0].size >= 500
        ]
        all_single = [
            get_time_dist(single, fs, dx, sel_ch, sel_ch_m)
            for single in single_list
            if single[0].size >= 500
        ]
        detected = (
            np.vstack(all_pairs + all_single)
            if all_pairs or all_single
            else np.empty((0, 2))
        )
        ...
        if len(detected) > 0:
            ax.scatter(
                detected[:, 0], detected[:, 1], c="tab:blue", s=10, label="Detections"
            )
...
plt.savefig(
    f"../figs/evaluation_{method}_{timestamp}_batch{batch}.pdf",
    bbox_inches=None,
    transparent=True,
)
...
for batch in tqdm(batches, desc="Processing annotated batches"):
    ...
    for assoc_path in assoc_files:
        timestamp = (
            os.path.basename(assoc_path).replace("association_", "").replace(".pkl", "")
        )
        ...
        # Load annotations
        try:
            n_annotations = pd.read_csv(
                f"{annot_root}/{batch.replace('Batch_', 'Batch')}/annotated_calls_north_{timestamp}.csv"
            )
            s_annotations = pd.read_csv(
                f"{annot_root}/{batch.replace('Batch_', 'Batch')}/annotated_calls_south_{timestamp}.csv"
            )
        ...
        if plot_figs:
            # Save the figures
            plt.savefig(
                f"../evaluation/{batch}/association_{timestamp}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
for batch in batches:
    if batch in confusion:
        ...
        batch_precision = (
            batch_totals["TP"] / (batch_totals["TP"] + batch_totals["FP"])
            if (batch_totals["TP"] + batch_totals["FP"]) > 0
            else 0
        )
        batch_recall = (
            batch_totals["TP"] / (batch_totals["TP"] + batch_totals["FN"])
            if (batch_totals["TP"] + batch_totals["FN"]) > 0
            else 0
        )
        batch_f1 = (
            2 * (batch_precision * batch_recall) / (batch_precision + batch_recall)
...
bars1 = axs[0].bar(
    x_pos,
    precision_vals,
    color=[batch_color] * (len(batch_names) - 1) + [overall_color],
    alpha=0.8,
    edgecolor="white",
    linewidth=1.5,
)
...
for ax, vals, bars in zip(
    axs, [precision_vals, recall_vals, f1_vals], [bars1, bars2, bars3]
):
