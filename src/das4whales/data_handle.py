...
def get_metadata_optasense(filepath: str) -> dict[str, Any]:
    ...
    if os.path.exists(filepath):
        ...
        with h5py.File(filepath, "r") as fp1:
            ...
            fs = fp1["Acquisition"]["Raw[0]"].attrs[
                "OutputDataRate"
            ]  # sampling rate in Hz
            dx = fp1["Acquisition"].attrs[
                "SpatialSamplingInterval"
            ]  # channel spacing in m
            ns = fp1["Acquisition"]["Raw[0]"]["RawDataTime"].attrs["Count"]
            n = fp1["Acquisition"]["Custom"].attrs[
                "Fibre Refractive Index"
            ]  # refractive index
            GL = fp1["Acquisition"].attrs["GaugeLength"]  # gauge length in m
            nx = fp1["Acquisition"]["Raw[0]"].attrs[
                "NumberOfLoci"
            ]  # number of channels
            scale_factor = (
                (2 * np.pi) / 2**16 * (1550.12 * 1e-9) / (0.78 * 4 * np.pi * n * GL)
            )

            start_dist = (
                fp1["Acquisition"]["Custom"].attrs["Output Channel Start (CSU)"] * dx
            )
...
def get_metadata_onyx(filepath: str) -> dict[str, Any]:
    ...
    if os.path.exists(filepath):
        ...
        start_dist = (
            0  # TODO: get this from metadata if possible, for now  assume it is 0
        )
        end_dist = nx * dx
        print(
            "WARNING: start_dist and end_dist are set to 0 and nx*dx respectively, please check if this is correct for your data"
        )
        print(
            "contact esnyder@cornell.edu if you would like to have this implemented for your data"
        )
        meta_data = {
            "fs": fs,
            "dx": dx,
...
def raw2strain(trace: np.ndarray, metadata: dict[str, Any]) -> np.ndarray:
    ...
    trace -= np.mean(
        trace, axis=1, keepdims=True
    )  # using np.median() is also possible, depending on the nature of the noise
...
def load_das_data(
    ...
    if interrogator in ["optasense", "silixa", "onyx"]:
        ...
        with h5py.File(filename, "r") as fp:
            ...
            # Handle datetime
            raw_data_time = str(
                fp["Acquisition"]["Raw[0]"]["RawDataTime"].attrs["PartStartTime"],
                encoding="ascii",
            )

            try:
                if "T" in raw_data_time:
                    file_begin_time_utc = datetime.fromisoformat(
                        raw_data_time.replace("Z", "+00:00")
                    )
                else:
                    file_begin_time_utc = datetime.strptime(
                        raw_data_time, "%Y-%m-%d %H:%M:%S"
                    )
            except ValueError:
    ...
    dist = (np.arange(nnx) * selected_channels[2] + selected_channels[0]) * metadata[
        "dx"
    ]
        ...
        with h5py.File(filepath, "r") as fp:
            ...
            if trace is None:
                ...
                file_begin_time_utc = datetime.fromtimestamp(
                    raw_data_time[index] * 1e-6, tz=timezone.utc
                )
            ...
            trace = (
                selected_trace
                if trace is None
                else da.concatenate([trace, selected_trace], axis=1)
            )
...
def generate_file_list(base_url: str, start_file: str, duration: int) -> list[str]:
    ...
    while accumulated_time < duration:
        ...
        next_filename = re.sub(
            r"(\d{4}-\d{2}-\d{2}T\d{6})Z",
            next_time.strftime("%Y-%m-%dT%H%M%S") + "Z",
            current_file,
        )
